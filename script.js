...
const highestElement = document.querySelector('.highestElement')
...
async function displayWeatherData(weatherObject, locationToDisplay) {
  ...
  temperatureHigh.innerText = `High: ${weatherObject.temperatureMax.toFixed(
    1
  )}°`
  temperatureLow.textContent = `Low: ${weatherObject.temperatureMin.toFixed(
    1
  )}°`
  humidity.textContent = `Humidity: ${weatherObject.humidity}%`

  windSpeed.textContent = `Wind speed: ${weatherObject.windSpeed.toFixed(
    1
  )} m/s`
  ...
  day1rainChance.textContent = `${Math.round(
    weatherObject.day1rainChance * 100
  )}% chance of rain`
  ...
  day2rainChance.textContent = `${Math.round(
    weatherObject.day2rainChance * 100
  )}% chance of rain`
  ...
  day3rainChance.textContent = `${Math.round(
    weatherObject.day3rainChance * 100
  )}% chance of rain`
  ...
  day4rainChance.textContent = `${Math.round(
    weatherObject.day4rainChance * 100
  )}% chance of rain`
  ...
  day5rainChance.textContent = `${Math.round(
    weatherObject.day5rainChance * 100
  )}% chance of rain`
  ...
  day6rainChance.textContent = `${Math.round(
    weatherObject.day6rainChance * 100
  )}% chance of rain`
  ...
  day7rainChance.textContent = `${Math.round(
    weatherObject.day7rainChance * 100
  )}% chance of rain`
}
...
async function getWeatherDataFromAPI(locationToDisplay) {
  ...
  highestElement.classList.add('highestElement--horizontal')
  outerContainer.appendChild(loadingGif)
  document.querySelector('.location').classList.add('location--horizontal')

  // Get details on CURRENT weather
  try {
    let currentResponse = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${locationToDisplay}&APPID=34e4d38b468e06f31319903dfdd2b111&units=metric`,
      { mode: 'cors' }
    )
    let currentWeatherData = await currentResponse.json()

    // Get details on FORECASTED weather
    let forecastedResponse = await fetch(
      `https://api.openweathermap.org/data/2.5/onecall?lat=${currentWeatherData.coord.lat}&lon=${currentWeatherData.coord.lon}&APPID=34e4d38b468e06f31319903dfdd2b111&units=metric`,
      { mode: 'cors' }
    )
